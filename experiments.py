...
from pipeline import Pix2Pix
from lasagne.nonlinearities import *
from lasagne.updates import *
from lasagne.utils import floatX
from utils import Hdf5Iterator
...
def get_iterators(dataset, batch_size, is_a_grayscale, is_b_grayscale, da=True):
    ...
    if da:
        imgen = ImageDataGenerator(
            horizontal_flip=True, vertical_flip=True, rotation_range=360, fill_mode="reflect")
    else:
        imgen = ImageDataGenerator()
    it_train = Hdf5Iterator(dataset['xt'], dataset['yt'], batch_size,
                            imgen, is_a_grayscale=is_a_grayscale, is_b_grayscale=is_b_grayscale)
    it_val = Hdf5Iterator(dataset['xv'], dataset['yv'], batch_size, imgen,
                          is_a_grayscale=is_a_grayscale, is_b_grayscale=is_b_grayscale)
...
if __name__ == '__main__':
    ...
    def test1_nobn(mode):
        ...
        data = "/Procedural-Map-Generation/Data/data.h5"
        bs = 4
        it_train, it_val = get_iterators(data, bs, True, False, True)
        name = "test1_repeatnod_fixp2p_nobn"
        if mode == "train":
            model.train(it_train, it_val, batch_size=bs, num_epochs=1000,
                        out_dir="output/%s" % name, model_dir="models/%s" % name)
        elif mode == "interp":
            ...
            model.generate_interpolation(
                z1, z2, "/tmp/test.png", mode='matrix')
        elif mode == "gen":
            model.load_model("models/%s/600.model.bak" % name)
    ...
    def test1_nobn_finetunep2p_bilin(mode):
        ...
        # only load the dcgan
        model.load_model(
            "models/test1_repeatnod_fixp2p_nobn/1000.model.bak", mode='dcgan')
        data = "/Procedural-Map-Generation/Data/data.h5"
        bs = 4
        it_train, it_val = get_iterators(data, bs, True, False, True)
        ...
        if mode == "train":
            ...
            model.load_model(
                "models/test1_repeatnod_fixp2p_nobn/1000.model.bak", mode='dcgan')
            model.load_model(
                "models/test1_repeatnod_fixp2p_nobn_finetunep2p_bilin/1000.model.bak", mode='p2p')
            model.generate_interpolation_clip(
                100, 4, "output/%s/interp_clip_600_concat_bothdet/" % name, concat=True, deterministic=True)

    # bilinear upsampling with the dcgan
