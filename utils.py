...
def iterate_hdf5(imgen=None, is_a_grayscale=True, is_b_grayscale=False, is_uint8=True):
    def _iterate_hdf5(X_arr, y_arr, bs, rnd_state=np.random.RandomState(0)):
        ...
        while True:
            ...
            for elem in slices:
                this_X, this_Y = X_arr[elem].astype(
                    "float32"), y_arr[elem].astype("float32")
                ...
                if is_uint8:
                    this_X = (
                        this_X / 255.0) if is_a_grayscale else (this_X - 127.5) / 127.5
                    this_Y = (
                        this_Y / 255.0) if is_b_grayscale else (this_Y - 127.5) / 127.5
                # if we passed an image generator, augment the images
                if imgen != None:
                    seed = rnd_state.randint(0, 100000)
                    this_X = imgen.flow(
                        this_X, None, batch_size=bs, seed=seed).next()
                    this_Y = imgen.flow(
                        this_Y, None, batch_size=bs, seed=seed).next()
                yield this_X, this_Y
    return _iterate_hdf5

# Wrapper for the HDF5 iterator
...
class Hdf5Iterator():
    def __init__(self, X, y, bs, imgen, is_a_grayscale, is_b_grayscale, is_uint8=True):
        ...
        self.fn = iterate_hdf5(imgen, is_a_grayscale,
                               is_b_grayscale, is_uint8)(X, y, bs)
        self.N = X.shape[0]
...
def plot_grid(out_filename, itr, out_fn, is_a_grayscale, is_b_grayscale, N=4):
    ...
    for i in range(N*N):
        ...
        if out_fn != None:
            ...
            bp = b
        img = compose_imgs(
            a[0], bp[0], is_a_grayscale=is_a_grayscale, is_b_grayscale=is_b_grayscale)
        plt.subplot(N, N, i+1)
        plt.imshow(img)
        plt.axis('off')
