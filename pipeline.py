from utils import convert_to_rgb, compose_imgs, plot_grid
import gzip
import pickle
from keras_equivalent import ReduceLROnPlateau
...
class Pix2Pix():
    ...
    """
        Two-stage DCGAN/pix2pix GAN. 

        Given training data (pairs) in the form [A,B], the DCGAN maps from prior samples z -> A, and the pix2pix GAN synthesises B images from A images.
    ...
    def __init__(self,
        ...
        dcgan_gen = gen_fn_dcgan(
            latent_dim, is_a_grayscale, **gen_params_dcgan)
        dcgan_disc = disc_fn_dcgan(in_shp, is_a_grayscale, **disc_params_dcgan)
        # get the networks for the p2p network
        p2p_gen = gen_fn_p2p(in_shp, is_a_grayscale,
                             is_b_grayscale, **gen_params_p2p)
        p2p_disc = disc_fn_p2p(in_shp, is_a_grayscale,
                               is_b_grayscale, **disc_params_p2p)
        if verbose:
            print "p2p gen:"
        ...
        dcgan['disc_out_fake'] = get_output(
            dcgan_disc, dcgan['gen_out'])  # D(G(z))
        # construct theano stuff for the p2p gen/disc
        p2p = {'gen': p2p_gen, 'disc': p2p_disc["out"]}
        p2p['disc_out_real'] = get_output(
            p2p_disc["out"], {p2p_disc["inputs"][0]: X, p2p_disc["inputs"][1]: Y})  # D(X,Y)
        ...
        p2p['disc_out_fake'] = get_output(p2p_disc["out"], {
                                          p2p_disc["inputs"][0]: X, p2p_disc["inputs"][1]: p2p['gen_out']})  # D(X, X_to_y(X))
        if lsgan:
        ...
        disc_loss_dcgan = adv_loss(dcgan['disc_out_real'], 1.).mean(
        ) + adv_loss(dcgan['disc_out_fake'], 0.).mean()
        ...
        disc_loss_p2p = adv_loss(p2p['disc_out_real'], 1.).mean(
        ) + adv_loss(p2p['disc_out_fake'], 0.).mean()
        # dcgan params
        ...
        if train_mode == 'both':
            ...
            # update dcgan discriminator
            updates.update(opt(disc_loss_dcgan, disc_params_dcgan, **opt_args))
            # update p2p generator
            updates.update(opt(gen_total_loss_p2p, gen_params_p2p, **opt_args))
            # update p2p discriminator
            updates.update(opt(disc_loss_p2p, disc_params_p2p, **opt_args))
            ...
            # update p2p discriminator
            updates.update(opt(disc_loss_p2p, disc_params_p2p, **opt_args))
        train_fn = theano.function([Z, X, Y], [gen_loss_dcgan, disc_loss_dcgan, gen_loss_p2p,
                                               recon_loss, disc_loss_p2p], updates=updates, on_unused_input='warn')
        loss_fn = theano.function([Z, X, Y], [gen_loss_dcgan, disc_loss_dcgan,
                                              gen_loss_p2p, recon_loss, disc_loss_p2p], on_unused_input='warn')
        ...
        self.train_keys = ['dcgan_gen', 'dcgan_disc',
    ...
    def train(self, it_train, it_val, batch_size, num_epochs, out_dir, model_dir=None, save_every=10, resume=False, quick_run=False):
        ...
        def _loop(fn, itr):
            ...
            for b in range(itr.N // batch_size):
                ...
                Z_batch = floatX(self.sampler(
                    X_batch.shape[0], self.latent_dim))
        ...
        if self.verbose:
            try:
                ...
                draw_to_file(get_all_layers(
                    self.dcgan['gen']), "%s/gen_dcgan.png" % out_dir, verbose=True)
                draw_to_file(get_all_layers(
                    self.dcgan['disc']), "%s/disc_dcgan.png" % out_dir, verbose=True)
                draw_to_file(get_all_layers(
                    self.p2p['gen']), "%s/gen_p2p.png" % out_dir, verbose=True)
                draw_to_file(get_all_layers(
                    self.p2p['disc']), "%s/disc_p2p.png" % out_dir, verbose=True)
            except:
                pass
        ...
        for e in range(num_epochs):
            ...
            if self.train_mode in ['both', 'p2p']:
                ...
                plot_grid("%s/out_%i.png" % (out_dir, e+1), it_val, self.gen_fn,
                          is_a_grayscale=self.is_a_grayscale, is_b_grayscale=self.is_b_grayscale)
                # plot big pictures of predict(A) in the valid set
                self.generate_atob(it_train, 1, "%s/dump_train" %
                                   out_dir, deterministic=False)
                self.generate_atob(it_val, 1, "%s/dump_valid" %
                                   out_dir, deterministic=False)
            if self.train_mode in ['both', 'dcgan']:
                # plot A generated from G(z)
                self.generate_gz(num_examples=20, batch_size=batch_size,
                                 out_dir="%s/dump_a" % out_dir, deterministic=False)
    ...
    def generate_atob(self, itr, num_batches, out_dir, dont_predict=False, deterministic=True):
        ...
        for n in range(num_batches):
            ...
            for i in range(pred_y.shape[0]):
                ...
                imsave(fname="%s/%i.a.png" %
                       (out_dir, ctr), arr=this_x_processed)
                imsave(fname="%s/%i.b.png" %
                       (out_dir, ctr), arr=pred_y_processed)
                ctr += 1
    ...
    def generate_interpolation(self, out_name, zsample1=None, zsample2=None, deterministic=True, mode='row', figsize=(10, 10), cmap='gray'):
        ...
        if mode == 'row':
            grid = np.zeros((1, 6, self.in_shp, self.in_shp,
                             1 if self.is_a_grayscale else 3), dtype=zsample1.dtype)
        else:
    ...
    def generate_interpolation_clip(self, num_samples, batch_size, out_dir, deterministic=True, min_max_norm=False, concat=False):
        ...
        for b in range(all_tps.shape[0] // batch_size):
            ...
            for i in range(z_out.shape[0]):
                ...
                this_a_img = convert_to_rgb(
                    this_a_img, is_grayscale=self.is_a_grayscale)
                this_b_img = convert_to_rgb(
                    this_b_img, is_grayscale=self.is_b_grayscale)
                d = '%04d' % ctr
                if concat:
                    full_img = np.zeros(
                        (self.in_shp, self.in_shp*2, 3), dtype=zs.dtype)
                    full_img[0:self.in_shp, 0:self.in_shp, :] = this_a_img
                    full_img[0:self.in_shp, self.in_shp::, :] = this_b_img
                    imsave(arr=full_img, fname="%s/concat_%s.png" %
                           (out_dir, d))
                else:
